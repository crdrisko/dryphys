...
DryPhys::Vector3D ScenePlay::gridToMidPixel(float gridX, float gridY, EntityPtr entity)
{
...
void ScenePlay::loadLevel(const std::string& filename)
{
    ...
    entityManager_ = Engine2D::EntityManager();
    ...
    for (const auto& row : rows)
    {
        ...
        if (splitRow[0] == "Tile")
        {
             ...
             *  Grid Y Pos:         GY      float
             */
            EntityPtr tileEntity = entityManager_.addEntity("tile");
            ...
            tileEntity->addComponent<CTransform>(gridToMidPixel(std::stof(splitRow[2]), std::stof(splitRow[3]), tileEntity),
                DryPhys::Vector3D {},
        ...
        else if (splitRow[0] == "Dec")
        {
             ...
             *  Grid Y Pos:         GY      float
             */
            EntityPtr tileEntity = entityManager_.addEntity("dec");
        ...
        {
            EntityPtr envEntity = entityManager_.addEntity("env");
        ...
        {
            music_ = std::move(game_->assets().getMusic(splitRow[1]));
...
{
    EntityPtr enemyEntity = entityManager_.addEntity("enemy");
...
void ScenePlay::spawnBullet(EntityPtr spawner) {}
...
void ScenePlay::sMovement()
{
    ...
    for (EntityPtr entity : entityManager_.getEntities())
    {
...
void ScenePlay::sCollision()
{
    ...
    for (EntityPtr tile : entityManager_.getEntities("tile"))
    {
...
{
    auto& pAnim  = player_->getComponent<CAnimation>();
    auto pState  = player_->getComponent<CState>().state;
    auto& pInput = player_->getComponent<CInput>();

    if (pAnim.animation.getName() == playerConfig_.AVATAR + "Idle")
    {
        switch (pState->queryState())
        {
        case pState->Walking:
            player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "Walk"));
            break;
        case pState->Running:
            player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "Run"));
            break;
        case pState->Climbing:
            player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "Climb"));
            break;
        case pState->Evading:
            player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "Evade"));
            break;
        case pState->Jumping:
            if (pInput.canJump)
                player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "Jump"));
            break;
        case pState->Attacking:
            player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "Shoot"));
            break;
        case pState->Defending:
            player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "Crouch"));
            break;
        case pState->Hurting:
            player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "Hurt"));
            break;
        default:
            break;
        }
    }
    else if (pAnim.animation.getName() == playerConfig_.AVATAR + "Walk")
    {
        switch (pState->queryState())
        {
        case pState->Idling:
        ...
        case pState->Jumping:
            ...
            break;
        case pState->Attacking:
            player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "RunShoot"));
            break;
        case pState->Defending:
            player_->addComponent<CAnimation>(game_->assets().getAnimation(playerConfig_.AVATAR + "Crouch"));
            break;
    ...
    else if (pAnim.animation.getName() == playerConfig_.AVATAR + "Run")
    {
        switch (pState->queryState())
        {
        case pState->Idling:
    ...
    else if (pAnim.animation.getName() == playerConfig_.AVATAR + "Jump")
    {
    ...
    else if (pAnim.animation.getName() == playerConfig_.AVATAR + "Evade")
    {
        if (pAnim.animation.hasEnded())
        {
            switch (pState->queryState())
    ...
    else if (pAnim.animation.getName() == playerConfig_.AVATAR + "Climb")
    {
        switch (pState->queryState())
        {
        case pState->Idling:
    ...
    else if (pAnim.animation.getName() == playerConfig_.AVATAR + "Crouch")
    {
        switch (pState->queryState())
        {
        case pState->Idling:
    ...
    else if (pAnim.animation.getName() == playerConfig_.AVATAR + "Hurt")
    {
        switch (pState->queryState())
        {
        case pState->Idling:
    ...
    else if (pAnim.animation.getName() == playerConfig_.AVATAR + "Shoot"
             || pAnim.animation.getName() == playerConfig_.AVATAR + "RunShoot")
    {
        switch (pState->queryState())
        {
        case pState->Idling:
...
void ScenePlay::sRender()
{
    ...
    if (drawTextures_)
    {
        for (EntityPtr e : entityManager_.getEntities())
        {
